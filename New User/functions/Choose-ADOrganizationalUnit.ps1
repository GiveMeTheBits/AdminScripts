...
function Choose-ADOrganizationalUnit{
	...
	function Get-NextLevel
	{
		...
		If ($Type -eq 'Domain')
		{
			$ADObjects = $forest.domains | Where-Object{$_.Name -eq $RootNode.Text} | #EDIT ? to Where-Object
				Select-Object -ExpandProperty Children #EDIT select to select-object
			$RootNode.Nodes.Clear()
	        $ADObjects | ForEach-Object { #EDIT % to ForEach-Object
		...
		else
		{
			...
			{
				$ADObjects = $ADsearcher.FindAll() | Where-Object {$_.Properties['showinadvancedviewonly'][0] -eq $false -or 
	    ...
	    $buttonOK_Click={
		    ...
		    try
		    {
                ...
                If ($CB_UseCred.Checked)
			    {
                    ...
                    $SecPwd = $Password | ConvertTo-SecureString -Force -AsPlainText
			    ...
			    $DomainContext = New-Object System.DirectoryServices.ActiveDirectory.DirectoryContext("Domain", $DomainEntry.name, $UserName, $Password) #EDIT Add user and Pass to Strings
			    $Domain = [System.DirectoryServices.ActiveDirectory.Domain]::GetDomain($DomainContext)
			    $ForestContext = New-Object System.DirectoryServices.ActiveDirectory.DirectoryContext("Forest", $domain.forest, $UserName, $Password) #EDIT Add user and Pass to Strings
