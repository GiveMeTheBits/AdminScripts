$SearchThing = Get-ADObject -Filter {samaccountname -eq 'GRAYMIC'} -Properties objectsid
$SearchDN = $SearchThing.DistinguishedName
...
$bSearch = New-Object System.DirectoryServices.DirectoryEntry("LDAP://$($domainController.Name)/$SearchDN") #connect to DOMAINCONTROLLER using LDAP path
