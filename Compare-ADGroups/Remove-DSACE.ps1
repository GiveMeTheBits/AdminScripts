...
$DNs = Get-ADObject -SearchScope OneLevel -SearchBase "DC=Corp,DC=Fleishman,DC=Com" -Filter {(ObjectClass -eq "organizationalUnit") -or (ObjectClass -eq "container")}

$SearchFor = 'cis','cis-lc','Acct-Admins','Apac-Mac','Can-Cis','Eur-Cis','Gmw-Cis','Helpdesk-Setup','ROLE-IT-ATS','HPE-Endpoint-WIN','HPE-Security-Antivirus','HPE-Security-Encryption','HPE-Site-Support','LatAm-Cis','Networking','Tex-Cis','Westcoast-Cis','Tor-Cis'
$Groups = $SearchFor | get-adgroup | select SamAccountName | ForEach-Object {"Corp\$($_.SamAccountName)"}

#multi Group Lookup
$V = Foreach ($DN in $DNs){Remove-DSACE -Report -Path $DN.DistinguishedName -Trustee $Groups -Recurse -ErrorAction SilentlyContinue};$v | Out-GridView
#Single Group Lookup
$D = Foreach ($DN in $DNs){Remove-DSACE -Report -Path $dn.DistinguishedName -Trustee "corp\can-cis-lc" -Recurse};$D | Out-GridView